	...
	while (true) {
		...
		case 1: // Quan li sach
			while (true) {
				...
				case 2:
				{
					...
					string masach;
					cin >> masach;
					if (masach.length() != 6 || masach.substr(0, 2) != "MS" || !isNumber(masach.substr(2, 4))) {
						setColor(4);
		...
		case 3: // Quan li ban doc
			while (true) {
				...
				case 3:
				{
					if (dsBanDoc.getSize() == 0) {
						setColor(4);
						cout << "\t\t\t\t\tChua co ban doc nao dang ki!";
					}
					system("cls");
					setColor(7);
...
/******************************************************************************************************
 * @ Description   Kiểm tra 1 chuỗi có phải là số
